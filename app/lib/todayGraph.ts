...
/**
 * A monotone cubic through the points, as an SVG path.
 *
 * A polyline through forty columns is a saw, and the corner at every column is
 * an artefact of sampling rather than anything the player did. Fritsch–Carlson
 * is the smoothing that is allowed here: it interpolates the points exactly and
 * its tangents are clamped so a curve can never overshoot the samples it joins.
 * A Catmull–Rom or a plain cardinal spline would round the same corners and
 * invent a dip below the lowest quartile on the way, which on this figure would
 * be a position no game was ever in.
 */
export function smoothPath(
  ...
  if (n === 1) return `${command}${at(0).x.toFixed(1)} ${at(0).y.toFixed(1)}`;

  // Secant slopes between neighbours, then tangents clamped to them.
  const slope: number[] = [];
  for (let i = 0; i < n - 1; i++) {
    const dx = at(i + 1).x - at(i).x;
    slope.push(dx === 0 ? 0 : (at(i + 1).y - at(i).y) / dx);
  }
  ...
  let d = `${command}${at(0).x.toFixed(1)} ${at(0).y.toFixed(1)}`;
  for (let i = 0; i < n - 1; i++) {
    const p0 = at(i);
    const p1 = at(i + 1);
    const dx = (p1.x - p0.x) / 3;
    d += ` C${(p0.x + dx).toFixed(1)} ${(p0.y + dx * tangent[i]!).toFixed(1)}`;
    d += ` ${(p1.x - dx).toFixed(1)} ${(p1.y - dx * tangent[i + 1]!).toFixed(1)}`;
